...
SUM <- c(nSUM,nSBG5p,nfull,nSBG,nGDM,nICG)
RATIO <- c(rSUM,rSBG5p,rfull,rSBG,rGDM,rICG)
x = data.frame(SUM,RATIO)
namer <- basename(getwd())
colnames(x) <- c(paste0(namer,"_NUMS"),paste0(namer))
rownames(x) <- c("SUM","SBG5p","FULL","SBG", "GDM","ICG")
write.csv(x,paste0(namer,"_list.csv"),sep="",row.names=T,col.names=T)
}
...
plot <- function(t) {
        ...
        # geom_point(data=uSB, aes(x=variable, y=value, group=1)) + 
        # geom_line(data=uSB, aes(x=variable, y=value, group=1), color="black",group=1) + 
        # geom_line(data=uSBp, aes(x=variable, y=value, group=1), linetype = "dashed",color="red",group=1) + 
        geom_line(data=SB, aes(x=variable, y=value, group=1), linetype = "dashed",color="red",group=1) + 
        theme(legend.title=element_blank()) +  
        theme (panel.border = element_blank()) +
        theme(legend.position="right") + 
        theme(legend.key = element_blank()) + 
        theme(legend.background = element_blank()) + 
        geom_text(data=SB, aes(label=value, y=value+5), position=position_dodge(0.9), vjust=0, size = 6, color="red") +  
        # geom_text(data=uSB, aes(label=value, y=value+5), position=position_dodge(0.9), vjust=0, size = 6, color="red") +  
        # geom_text(data=uSBp, aes(label=value, y=value+2), position=position_dodge(0.9), vjust=0, size = 6, color="red") +  
