## Aligned reads alignment output: {b*} STEP 1 ~ 4
...
cd t
cp ../all.F.fasta L01
## Choose BLAST alignment loop number. 
let num=5 # read -p "number of loop：" num;
b=1
let num1=$num-1
for i in $(seq 1 $num1)
do
...
do
    ...
    let jb=$j*2    
    echo "LS"
    python2 LS L$i1$j b$i1$j L$i$ja
    echo "RS"
    python2 RS L$i1$j b$i1$j L$i$jb
    fi
    done    
done
